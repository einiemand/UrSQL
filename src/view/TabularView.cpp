...
std::vector<std::size_t> calculateColumnWidth(
  const std::vector<std::string>& headers,
  const std::vector<std::vector<Value>>& valueRows) {
    ...
    for (auto& valueRow : valueRows) {
        URSQL_ASSERT(valueRow.size() == headers.size(),
                     "row size should match header size");
...
TabularView::TabularView(std::vector<std::string> headers,
...
void TabularView::show(std::ostream& os) const {
    ...
    std::vector<std::size_t> widths =
      calculateColumnWidth(headers_, valueRows_);
