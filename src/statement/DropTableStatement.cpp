...
DropTableStatement::DropTableStatement(std::vector<std::string> tableNames) : MultiTableStatement(std::move(tableNames)) {}
...
ExecuteResult DropTableStatement::run(DBManager& dbManager) const {
    ...
    return { std::make_unique<RowsAffectedTextView>(tableNames_.size()), false };
}

std::unique_ptr<DropTableStatement> DropTableStatement::parse(TokenStream& ts) {
    std::vector<std::string> tableNames = parser::parseCommaSeparated(ts, parser::parseNextIdentifier);
