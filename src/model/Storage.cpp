...
const size_type Storage::extensionLength =
  strlen(Storage::defaultFileExtension);

Storage::Storage(std::string aFileName, CreateNewFile, const TOC& aTOC,
                 StatusResult& aResult)
    : m_fileName(std::move(aFileName)) {
    aResult = _setupTOC(aTOC);
}

Storage::Storage(std::string aFileName, OpenExistingFile, TOC& aTOC,
                 StatusResult& aResult)
    : m_fileName(std::move(aFileName)) {
...
bool Storage::hasDefaultExtension(const std::string& aFileName) {
    return aFileName.substr(aFileName.length() - Storage::extensionLength) ==
           Storage::defaultFileExtension;
}
...
StatusResult Storage::_setupTOC(const TOC& aTOC) {
    ...
    return StatusResult(Error::write_error,
                        "File '" + theFilePath + "' cannot be opened");
}
...
StatusResult Storage::_loadTOC(TOC& aTOC) {
    ...
    return StatusResult(Error::read_error,
                        "File '" + m_fileName + "' cannot be opened");
}
...
StatusResult Storage::readBlock(Block& aBlock, blocknum_t aBlocknum) {
    ...
    if (m_file.seekg(static_cast<int64_t>(aBlocknum) * defaultBlockSize)) {
        if (!m_file.read(reinterpret_cast<char*>(&aBlock), defaultBlockSize)) {
            theResult.setError(Error::read_error,
                               "Unable to read blocks from '" +
...
StatusResult Storage::eachBlock(BlockVisitor aVisitor) {
    ...
    Block theBlock;
    for (blocknum_t theBlocknum = 0; theResult && theBlocknum < theBlockCnt;
         ++theBlocknum)
    {
