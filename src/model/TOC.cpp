...
void TOC::serialize(BufferWriter& aWriter) const {
    ...
    // theResult.setError(Error::block_fullData, "Block is full, cannot hold
    // more data from TOC");
}
...
blocknum_t TOC::getEntityPosByName(const std::string& anEntityName) const {
    URSQL_TRUTH(entityExists(anEntityName),
                "Check if entity exists before getting its block number!");
    return m_entityMap.at(anEntityName);
}

void TOC::add(const std::string& anEntityName, blocknum_t aBlocknum) {
    URSQL_TRUTH(!entityExists(anEntityName),
                "Check if entity exists before adding it to TOC!");
...
void TOC::drop(const std::string& anEntityName) {
    URSQL_TRUTH(entityExists(anEntityName),
                "Check if entity exists before dropping it from TOC!");
