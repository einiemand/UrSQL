...
public:
    ...
    Storage(std::string aFileName, CreateNewFile, const TOC& aTOC,
            StatusResult& aResult);

    Storage(std::string aFileName, OpenExistingFile, TOC& aTOC,
            StatusResult& aResult);
    ...
    StatusResult releaseBlocks(const IterableT& aBlocknumContainer) {
        ...
        for (auto iter = aBlocknumContainer.cbegin();
             theResult && iter != aBlocknumContainer.cend(); ++iter)
        {
